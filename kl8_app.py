...
def build_feature_engineer(df, rolling_window_list=[5, 10, 30]):
    """
    特征工程核心模块：基础特征+新增高级特征，严格滚动窗口计算（杜绝未来数据泄露）
    ...
    for idx, num in enumerate(LOTTERY_RULE["number_range"]):
        ...
        for model_name, model in model_dict.items():
            ...
            try:
                ...
                model_metrics[model_name].append(0.5)  # 极端情况默认0.5
...
# -------------------------- 3. Streamlit页面布局（含3个新增模块Tab） --------------------------
st.title("快乐8数据分析&预测系统V2.0 | 新增3大核心模块")
st.markdown("---")
...
with st.sidebar:
    ...
    st.markdown("*数据格式要求：第一列为【期号】，后续20列为【开奖号码1-20】*")
...
with tab2:
    ...
    if "raw_original_db" not in st.session_state:
        ...
        if st.button("一键生成全量特征（含高级特征）", type="primary"):
            ...
            st.success("全量特征生成完成！含基础特征+新增高级特征")
            col1, col2 = st.columns(2)
            with col1:
...
# ================================== Tab3：【新增模块2】高级特征分析 ==================================
with tab3:
    st.header("【新增模块2】高级特征分析中心")
    if "feature_engineered_db" not in st.session_state:
...
with tab4:
    ...
    if "feature_engineered_db" not in st.session_state:
        ...
        if st.button("一键训练单模型&生成预测", type="primary"):
            feature_df = st.session_state["feature_engineered_db"]
            feature_columns = st.session_state["feature_columns"]
...
with tab5:
    ...
    if "feature_engineered_db" not in st.session_state:
        ...
        if st.button("一键训练多模型&融合预测", type="primary"):
...
with tab6:
    ...
    with col1:
        ...
        if "hit_summary" not in st.session_state:
    ...
    with col2:
        ...
        if "multi_model_hit_summary" not in st.session_state:
...
with tab7:
    ...
    if predict_source == "单模型预测结果":
        ...
        if "final_predict_result" not in st.session_state:
    ...
    if predict_df is not None:
        if st.button("生成低重复率普通打票组合", type="primary"):
...
with tab8:
    ...
    if predict_df is not None:
        ...
        if st.button("生成低重复率胆拖组合", type="primary"):
...
# ====================== 【模块代码】快乐8数据分析预测底层库 V2.0 ======================
# 模块用途：快乐8全流程数据分析、特征工程、模型预测、组合生成，全对话可复用调用
# 存档状态：只读底层库，不可修改核心代码，自定义需求通过传参实现
# 导出能力：全量函数、常量、配置均可通过 import 直接调用
...
def load_standard_data(uploaded_file=None):
    """
    数据加载&标准化：只读原始底层库+衍生分析库分离
    调用示例：df = load_standard_data(uploaded_file)
    """
    if uploaded_file is None:
        mock_periods = 200
        mock_data = []
...
def build_feature_engineer(df, rolling_window_list=[5, 10, 30]):
    """
    全量特征工程：基础特征+高级特征（区间/奇偶/同尾/连号/斜连号）
    调用示例：feature_df, valid_feature_columns = build_feature_engineer(df, [5,10,30])
    """
    feature_df = df.copy(deep=True)
    total_periods = len(feature_df)
...
def feature_analysis(feature_df, feature_columns):
    """
    高级特征分析：特征相关性、方差分析
    调用示例：corr_df, feature_var = feature_analysis(feature_df, feature_columns)
    """
    corr_df = feature_df[feature_columns].corr()
    feature_var = feature_df[feature_columns].var().sort_values(ascending=False)
    return corr_df, feature_var
...
def train_predict_model(feature_df, feature_columns):
    """
    单模型训练&预测（XGBoost基准模型）
    调用示例：predict_df, hit_summary_df, accuracy_df = train_predict_model(feature_df, feature_columns)
    """
    total_samples = len(feature_df)
    train_size = int(total_samples * 0.7)
    val_size = int(total_samples * 0.15)
...
def train_multi_model(feature_df, feature_columns, model_weight_config=None):
    """
    多模型融合训练&预测（LR+RF+XGBoost+LightGBM）
    调用示例：final_predict_df, model_metrics_df, hit_summary_df = train_multi_model(feature_df, feature_columns)
    """
    if model_weight_config is None:
        model_weight_config = {
            "LogisticRegression": 0.15,
...
def generate_low_repeat_combinations(predict_df, select_count=8, group_count=5, max_repeat_rate=0.3):
    """
    低重复率普通打票组合生成
    调用示例：combo_df = generate_low_repeat_combinations(predict_df, 8, 5, 0.3)
    """
    top_pool = predict_df.head(select_count * 3)["号码"].tolist()
    combinations = []
...
def generate_dantuo_combinations(predict_df, dan_count=5, tuo_count=8, group_count=5, max_dan_repeat_rate=0.2, max_tuo_repeat_rate=0.4):
    """
    胆拖组合专属生成（适配5胆N拖、8+8等场景）
    调用示例：dantuo_df, dan_pool, tuo_pool = generate_dantuo_combinations(predict_df, 5, 8, 5)
    """
    dan_pool = predict_df.head(dan_count * 3)["号码"].tolist()
    tuo_pool = predict_df[~predict_df["号码"].isin(dan_pool)].head(tuo_count * 3)["号码"].tolist()
...
def build_streamlit_page():
    """
    一键构建完整Streamlit页面，全功能集成
    调用示例：build_streamlit_page()
    """
    st.set_page_config(
        page_title="快乐8数据分析预测系统V2.0",
        layout="wide",
    ...
    st.title("快乐8数据分析&预测系统V2.0 | 模块代码底层库")
    st.markdown("---")
    ...
    with tab4:
        ...
        if "feature_engineered_db" not in st.session_state:
            ...
            if st.button("一键训练单模型&生成预测", type="primary"):
                feature_df = st.session_state["feature_engineered_db"]
                feature_columns = st.session_state["feature_columns"]
                with st.spinner("模型训练&预测中..."):
                    predict_df, hit_summary_df, accuracy_df = train_predict_model(feature_df, feature_columns)
                st.success("单模型预测完成！")
                st.subheader("下期号码预测排名")
                col1, col2 = st.columns(2)
                with col1:
                    st.dataframe(predict_df, use_container_width=True)
                with col2:
    ...
    with tab7:
        ...
        if predict_source == "单模型预测结果":
            ...
            if "final_predict_result" in st.session_state:
                predict_df = st.session_state["final_predict_result"]
            else:
                st.warning("请先在【多模型融合优化】完成预测结果生成")
...
__all__ = [
    # 全局常量
    "LOTTERY_RULE", "CUSTOM_RULE",
    # 核心工具函数
    "load_standard_data", "build_feature_engineer", "feature_analysis",
    "train_predict_model", "train_multi_model",
    "generate_low_repeat_combinations", "generate_dantuo_combinations",
    # 页面集成函数
