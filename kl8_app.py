...
# Streamlit页面全局配置（必须放在所有页面渲染代码之前）
st.set_page_config(
...
# 全局存档目录配置
SAVE_DIR = "kl8_predict_save"
if not os.path.exists(SAVE_DIR):
    os.makedirs(SAVE_DIR)

# -------------------------- 2. 工具函数模块（全量核心函数，100%语法闭合） --------------------------
@st.cache_data(ttl=3600)  # 缓存提速，避免重复计算，1小时自动过期
...
# -------------------------- 3. Streamlit页面布局（全量Tab+交互逻辑，100%闭合） --------------------------
...
with tab1:
    ...
    with col1:
        ...
        st.download_button(
            label="下载原始底层库CSV",
            data=st.session_state["raw_original_db"].to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
    ...
    if "raw_original_db" not in st.session_state:
        ...
        if st.button("一键生成全量特征（含高级特征）", type="primary", use_container_width=True):
            ...
            st.download_button(
                ...
                data=feature_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
...
with tab3:
    ...
    if "feature_engineered_db" not in st.session_state:
        ...
        with col1:
            ...
            st.dataframe(feature_var, use_container_width=True, column_config={"value": "特征方差"}, height=400)
        ...
        st.download_button(
            label="下载高级特征明细CSV",
            data=feature_df[[col for col in feature_df.columns if any(keyword in col for keyword in ["区间", "奇偶", "尾号", "连号", "斜连"])]].to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
...
with tab4:
    ...
    if "feature_engineered_db" not in st.session_state:
        ...
        if st.button("一键训练单模型&生成预测", type="primary", use_container_width=True):
            ...
            st.download_button(
                label="下载单模型预测结果CSV",
                data=predict_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
...
with tab5:
    ...
    if "feature_engineered_db" not in st.session_state:
        ...
        if st.button("一键训练多模型&融合预测", type="primary", use_container_width=True):
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.dataframe(final_predict_df, use_container_width=True, height=400)
            with col2:
                st.subheader("Top20高概率推荐号码")
            ...
            st.download_button(
                ...
                data=final_predict_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
    ...
    with col1:
        ...
        if "hit_summary" not in st.session_state:
            ...
            st.download_button(
                ...
                data=hit_summary_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
    ...
    with col2:
        ...
        if "multi_model_hit_summary" not in st.session_state:
            ...
            st.download_button(
                ...
                data=multi_hit_summary_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8"),
...
with tab7:
    ...
    if predict_df is not None:
        if st.button("生成低重复率普通打票组合", type="primary", use_container_width=True):
            ...
            st.download_button(
                label="下载普通打票组合CSV",
                data=combo_df.to_csv(index=True, encoding="utf-8-sig").encode("utf-8"),
...
with tab8:
    ...
    if predict_df is not None:
        ...
        if st.button("生成低重复率胆拖组合", type="primary", use_container_width=True):
            ...
            st.download_button(
                label="下载胆拖组合CSV",
                data=dantuo_df.to_csv(index=True, encoding="utf-8-sig").encode("utf-8"),
